...
Status ReadTensorFromImageFile(const string& file_name, const int input_height,
  ...
  auto file_reader =
      Placeholder(root.WithOpName("image"), tensorflow::DataType::DT_STRING);
...
Status PrintPrediction(Tensor* output) 
{
  Eigen::TensorMap<Eigen::Tensor<int, 1, Eigen::RowMajor>,
                          Eigen::Aligned> prediction = output->flat<int>();
...
int main(int argc, char* argv[]) {
  ...
  string image = "tensorflow/examples/attention_ocr_predict/data/impasse.png";
  string graph = "tensorflow/examples/attention_ocr_predict/data/frozen_model.pb";
  string charset = "tensorflow/examples/attention_ocr_predict/data/charset_size=134.txt";
  int32 input_width = 600;
  int32 input_height = 150;
  string input_layer = "imageBatchPlaceholder";
  string output_layer = "AttentionOcr_v1/predicted_chars";
  string root_dir = "";
  ...
  if (!load_graph_status.ok()) {
    LOG(ERROR) << "Error in loading graph";
  ...
  Status read_tensor_status =
      ReadTensorFromImageFile(image_path, input_height, input_width, &resized_tensors);
  if (!read_tensor_status.ok()) {
    LOG(ERROR) << "Error in reading tensor from image file";
  ...
  Status print_status = PrintPrediction(&outputs[0]);
  if (!print_status.ok()) {
